...
head(test1[-1])
g.dist <- daisy(test1[, -1],
                    metric = "gower",
                    type = list(logratio = 62,symmetric=seq(1,61,by=1)))

plot(density(na.omit(g.dist[upper.tri(g.dist)])), main="kernel density")
plot(ecdf(g.dist[upper.tri(g.dist)]), main="ECDF")
library(dbscan)
kNNdistplot(g.dist, k=63)
abline(h=0.1,col='red')
##0.09 - 0.1 
detach("package:dbscan", unload=TRUE)
library(fpc)
dbc3 = dbscan(g.dist, eps=.1, MinPts=63, method="dist")
dbc3
dbc2
dbc2 = dbscan(g.dist, eps=.098, MinPts=63, method="dist")
## visualization 
hullplot(test1[-1], dbc2$cluster)
hullplot(test1[-1], dbc3$cluster)
