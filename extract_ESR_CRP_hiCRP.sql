...
--select top 10 * from nmh_cerner_dm.lab_result where event_dsc like '%sedimentation%'



select  mrd_pt_id, event_cd, event_dsc, result_val_num, event_start_dt_tm, result_units_dsc, ROW_NUMBER() over (partition by mrd_pt_id order by result_val_num ) as rk 
into #lab
from #table
